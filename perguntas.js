criaCartão(
    'biologia',
    'Qual é a função das mitocôndrias nas células?',
    'As mitocôndrias são responsáveis pela produção de energia na forma de ATP, através da respiração celular.')

criaCartão(
    'biologia',
    'O que é fotossíntese e qual a sua importância para os seres vivos?',
    'A fotossíntese é o processo pelo qual plantas, algas e algumas bactérias convertem luz solar em energia química, essencial para a produção de oxigênio e alimentos.')

criaCartão(
    'biologia',
    'Qual a diferença entre célula procarionte e eucarionte?',
    'Células procariontes não têm núcleo organizado, enquanto as células eucariontes possuem núcleo e organelas membranosas.')

criaCartão(
    'biologia',
    'Qual é a função do DNA no organismo?',
    'O DNA contém as instruções genéticas necessárias para o desenvolvimento e funcionamento de todos os seres vivos, e é responsável pela hereditariedade.')

criaCartão(
    'biologia',
    'O que são enzimas e qual é o seu papel nas reações químicas do corpo?',
    ' Enzimas são proteínas que catalisam reações químicas, acelerando-as sem serem consumidas no processo.')

criaCartão(
    'biologia',
    'Explique a teoria da seleção natural de Charles Darwin.',
    'A seleção natural é o processo pelo qual organismos mais adaptados ao ambiente têm maior chance de sobreviver e se reproduzir, transmitindo suas características.')

criaCartão(
    'biologia',
    'O que são ecossistemas e como os organismos interagem dentro deles?',
    'Ecossistemas são comunidades de organismos que interagem entre si e com o ambiente físico. As interações incluem competição, predação, simbiose, entre outro')

criaCartão(
    'biologia',
    'O que diferencia os animais vertebrados dos invertebrados?',
    ' Vertebrados possuem coluna vertebral e um esqueleto interno, enquanto os invertebrados não têm coluna vertebral.')

criaCartão(
    'biologia',
    'O que é o ciclo do nitrogênio e qual sua importância para os seres vivos?',
    ' O ciclo do nitrogênio é o processo de conversão do nitrogênio atmosférico em compostos utilizáveis pelas plantas. É crucial para a produção de proteínas e ácidos nucleicos.')

criaCartão(
    'biologia',
    'Como o sistema imunológico atua na defesa do corpo contra doenças?',
    'O sistema imunológico detecta e combate patógenos como bactérias e vírus, utilizando células de defesa, anticorpos e mecanismos de memória para proteger o corpo.')

criaCartão(
    'história',
    'Quem foi o primeiro imperador de Roma?',
    'Augusto (também conhecido como Otaviano) foi o primeiro imperador de Roma, governando de 27 a.C. até sua morte em 14 d.C.')

criaCartão(
    'história',
    'Qual foi o evento que marcou o início da Idade Média na Europa?',
    'A queda do Império Romano do Ocidente, em 476 d.C., é considerada o evento que marca o início da Idade Média.')

criaCartão(
    'história',
    'O que foi a Revolução Industrial?',
    'A Revolução Industrial foi um período de mudanças tecnológicas e econômicas no final do século XVIII e início do século XIX, quando a produção em fábricas e o uso de máquinas se tornaram predominantes, especialmente na Inglaterra.')

criaCartão(
    'história',
    'Qual foi a principal causa da Primeira Guerra Mundial?',
    'A principal causa foi o assassinato do arquiduque Franz Ferdinand, herdeiro do trono austro-húngaro, em 1914, o que desencadeou uma série de alianças militares e conflitos entre nações europeias.')

criaCartão(
    'história',
    'Quem liderou a luta pela independência da Índia?',
    'Mahatma Gandhi foi o principal líder da luta pela independência da Índia, utilizando métodos de resistência pacífica e desobediência civil.')

criaCartão(
    'história',
    'Qual foi o impacto da Revolução Francesa na história mundial?',
    'A Revolução Francesa (1789-1799) teve grande impacto, espalhando ideais de liberdade, igualdade e fraternidade, e influenciando outros movimentos revolucionários ao redor do mundo.')

criaCartão(
    'história',
    'Qual foi o tratado que encerrou a Primeira Guerra Mundial?',
    'O Tratado de Versalhes, assinado em 1919, encerrou formalmente a Primeira Guerra Mundial.')

criaCartão(
    'história',
    'Quem foi o responsável pela unificação da Alemanha no século XIX?',
    'Otto von Bismarck, chanceler da Prússia, foi o principal arquiteto da unificação alemã, concluída em 1871.')

criaCartão(
    'história',
    'O que foi a Guerra Fria?',
    'A Guerra Fria foi um período de tensões políticas, ideológicas e militares entre os Estados Unidos e a União Soviética, de 1947 até 1991, sem um confronto direto entre as superpotências.')

criaCartão(
    'história',
    'Quem descobriu o Brasil e em que ano isso aconteceu?',
    'O Brasil foi descoberto por Pedro Álvares Cabral, em 22 de abril de 1500.')